...
class TTSService:
    ...
    async def _convert_to_opus_frames(self, audio_bytes: bytes, file_type: str) -> bytes:
        ...
        try:
            ...
            # AudioSegment で PCM に変換 (暫定: 24kHz for stability)
            audio = AudioSegment.from_file(BytesIO(audio_bytes), format=file_type)
            audio = audio.set_channels(1).set_frame_rate(24000).set_sample_width(2)  # 暫定24kHz（安定性優先）
    ...
    async def _pcm_to_opus_frames(self, raw_data: bytes) -> list:
        ...
        try:
            ...
            logger.info(f"🎵 [TEMP_STABLE] Generated {frame_count} Opus frames (24kHz, 60ms, pure Opus) from {len(raw_data)} bytes PCM")
            return opus_frames_list
